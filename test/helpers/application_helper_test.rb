...
describe ApplicationHelper do
  ...
  describe "#paginate" do
    ...
    let(:request) { stub(script_name: "script", path_info: "path", GET: {}) }

    before { stubs(url_for: "foo") }

    it "does not show nav for 1-page" do
      paginate(*pagy(User.where("1=2"), page: 1, items: 1)).must_equal ""
    end

    it "shows records for paginate" do
      paginate(*pagy(User, page: 1, items: 1)).must_include " #{User.count} records"
    end

    it "does not show records for single-page" do
      paginate(*pagy(User, page: 1, items: 100)).wont_include " records"
    end

    it "does not show records for 0-page" do
      paginate(*pagy(User.where("1=2"), page: 1, items: 1)).wont_include " records"
    end
  end
